...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"ElegooSlicer_opengl_version_check";
        wc.style = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"ElegooSlicer_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0, wc.hInstance, (LPVOID)this);
            if (hwnd) {
...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    ...
    wcscat(path_to_slic3r, L"ElegooSlicer.dll");
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
    if (hInstance_Slic3r == nullptr) {
        printf("ElegooSlicer.dll was not loaded, error=%d\n", GetLastError());
        return -1;
    }

    // resolve function address here
    elegooslicer_main = (Slic3rMainFunc)GetProcAddress(hInstance_Slic3r,
#ifdef _WIN64
    ...
    if (elegooslicer_main == nullptr) {
        printf("could not locate the function elegooslicer_main in ElegooSlicer.dll\n");
        return -1;
    }
    // argc minus the trailing nullptr of the argv
    return elegooslicer_main((int)argv_extended.size() - 1, argv_extended.data());
}
}
