...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: ElegooSlicer is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::ElegooSlicer, SLIC3R_APP_NAME },
    { EProducer::ElegooSlicer, "generated by ElegooSlicer" },
    { EProducer::ElegooSlicer, "generated by BambuStudio" },
    { EProducer::ElegooSlicer, "BambuStudio" }
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by ElegooSlicer,
        // extract the config from it
        if (m_producer == EProducer::ElegooSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from ElegooSlicer's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::ElegooSlicer) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::ElegooSlicer: { return process_bambuslicer_tags(comment); }
...
void  GCodeProcessor::process_G2_G3(const GCodeReader::GCodeLine& line)
{
    ...
    // Elegoo: we now use spiral_vase_layers for proper layer detect when scarf joint is enabled,
