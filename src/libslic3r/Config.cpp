...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or ElegooSlicer header.
    ...
    {
        //BBS
        std::string bambuslicer_gcode_header = "; ElegooSlicer";

        std::string elegooslicer_gcode_header = std::string("; generated by ");
        elegooslicer_gcode_header += SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(elegooslicer_gcode_header.c_str(), line_c, strlen(elegooslicer_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
