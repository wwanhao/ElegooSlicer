...
namespace instance_check_internal
{
	...
	static BOOL CALLBACK EnumWindowsProc(_In_ HWND   hwnd, _In_ LPARAM lParam)
	{
		...
		if (wndTextString.find(L"ElegooSlicer") != std::wstring::npos && classNameString == L"wxWindowNR") {
	...
	static bool  send_message(const std::string &message_text, const std::string &version)
	{
		...
		if ( !checker->IsAnotherRunning() ) */
		{
			...
			std::string		interface_name = "com.elegoo3d.elegoo-slicer.InstanceCheck.Object" + version;
			...
			std::string		object_name = "/com/elegoo3d/ElegooSlicer/InstanceCheck/Object" + version;
...
void OtherInstanceMessageHandler::handle_message(const std::string& message) 
{
	...
	boost::regex re(R"(^(elegooslicer|prusaslicer|cura|bambustudio):\/\/open[\/]?\?file=)", boost::regbase::icase);
...
namespace MessageHandlerDBusInternal
{
	...
	static void respond_to_introspect(DBusConnection *connection, DBusMessage *request) 
	{
	    ...
	    const char  *introspection_data =
	        ...
	        "     </method>"
	        "   </interface>"
	        "   <interface name=\"com.elegoo3d.elegoo-slicer.InstanceCheck\">"
	...
	static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
	{
	    ...
	    std::string our_interface  = "com.elegoo3d.elegoo-slicer.InstanceCheck.Object" + wxGetApp().get_instance_hash_string();
...
void OtherInstanceMessageHandler::listen()
{
	...
	std::string			 interface_name = "com.elegoo3d.elegoo-slicer.InstanceCheck.Object" + instance_hash;
