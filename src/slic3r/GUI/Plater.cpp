    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    StateColor flush_bd_col(std::pair<wxColour, int>(wxColour(0, 120, 189), StateColor::Pressed),
                            std::pair<wxColour, int>(wxColour(0, 120, 189), StateColor::Hovered),
        ...
        try {
            if (type_3mf) {
                ...
                Semver             file_version;
                {
                    ...
                    //             show_info(q, _L("The 3mf is generated by old ElegooSlicer, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by ElegooSlicer, load geometry data only."), _L("Load 3mf"));
                    //     }
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on ElegooSlicer(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
