...
void Sidebar::sync_ams_list()
{
    ...
    if (unknowns > 0) {
        MessageDialog dlg(this,
            _L("There are some unknown filaments mapped to generic preset. Please update Elegoo Slicer or restart Elegoo Slicer to check if there is an update to system presets."),
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
        ...
        try {
            if (type_3mf) {
                ...
                Semver             file_version;
                {
                    ...
                    // BBS: version check
                    Semver app_version = *(Semver::parse(ELEGOOTechSupport_VERSION));
                    ...
                    //             show_info(q, _L("The 3mf is generated by old Elegoo Slicer, load geometry data only."), _L("Load 3mf"));
                    //         else
                    //             show_info(q, _L("The 3mf is not supported by ElegooSlicer, load geometry data only."), _L("Load 3mf"));
                    //     }
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - ElegooSlicer");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Elegoo Slicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to Elegoo Slicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Elegoo Slicer(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
