...
bool is_associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id             = L" Elegoo.Slicer.1";
...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("ElegooSlicer will terminate because of running out of memory."
        ...
        //throw;
     	...
     	BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        	flush_logs();
        wxString errmsg = _L("ElegooSlicer will terminate because of a localization error. "
                             "It will be appreciated if you report the specific scenario this issue happened.");
        wxMessageBox(errmsg + "\n\n" + wxString(ex.what()), _L("Critical error"), wxOK | wxICON_ERROR);
        std::terminate();
        //throw;
    } catch (const std::exception& ex) {
        wxLogError(format_wxstr(_L("ElegooSlicer got an unhandled exception: %1%"), ex.what()));
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        throw;
    }
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("ElegooSlicer ");
    if (start == std::string::npos) {
        start = body.find("ElegooSlicer ");
        if (start == std::string::npos)
            return boost::none;
    }
...
void GUI_App::init_webview_runtime()
{
    ...
    if (!WebView::CheckWebViewRuntime()) {
        int nRet = wxMessageBox(_L("Elegoo Slicer requires the Microsoft WebView2 Runtime to operate certain features.\nClick Yes to install it now."),
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
	...
	if (m_app_conf_exists) {
        ...
        if (!error.empty()) {
            // Elegoo: if the config file is corrupted, we will show a error dialog and create a default config file.
...
int GUI_App::OnExit()
{
    ...
    // Elegoo: clean up encrypted bbl network log file if plugin is used
...
bool GUI_App::on_init_inner()
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current ElegooSlicer Version %1%")%SoftFever_VERSION;
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "ElegooSlicer", wxICON_QUESTION | wxYES_NO);
    ...
    if (is_editor()) {
            ...
            if (this->plater_ != nullptr) {
               ...
               /* wxString tips = wxString::Format(_L("Click to download new version in default browser: %s"), version_info.version_str);
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("New version of Elegoo Slicer"),
                    false,
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("The Elegoo Slicer needs an upgrade"),
                    false,
                    wxCENTER | wxICON_INFORMATION);
...
void GUI_App::UpdateDarkUI(wxWindow* window, bool highlited/* = false*/, bool just_font/* = false*/)
{
    ...
    if (m_is_dark_mode) {
        ...
        // * the background color of a button: #0066FF  -- 73
        if (bg_col != orig_col) {
...
void GUI_App::on_http_error(wxCommandEvent &evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(nullptr, _L("The version of Elegoo Slicer is too low and needs to be updated to the latest version before it can be used normally"), "", wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {
        }
...
{
    // Elegoo: alaways disable track event
    if (m_agent) {
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by ElegooSlicer.conf: %1%") % language;
        else {
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by ElegooSlicer: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
    	...
    	wxString message = "Switching Elegoo Slicer to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "Elegoo Slicer - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto ElegooSlicer icon in Finder,
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Elegoo.Slicer.1";
    std::wstring prog_desc = L"ElegooSlicer";
