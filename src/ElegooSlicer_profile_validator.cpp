...
void generate_custom_presets(PresetBundle* preset_bundle, AppConfig& app_config)
{
    ...
    auto createCustomPrinters = [&](Preset::Type type) {
        ...
        for (auto& parent : collection->get_presets()) {
            if (!parent.is_system)
                continue;
            auto new_name = parent.name + "_elegoo_test";
            if (parent.vendor)
        ...
        for (auto p : custom_preset) {
            ...
            if (type == Preset::TYPE_FILAMENT)
                parent->config.set_key_value("filament_start_gcode",
                                                 new ConfigOptionStrings({"this_is_elegoo_test_filament_start_gcode_mock"}));
            else if (type == Preset::TYPE_PRINT)
                parent->config.set_key_value("filename_format", new ConfigOptionString("this_is_elegoo_test_filename_format_mock"));
            else if (type == Preset::TYPE_PRINTER)
                parent->config.set_key_value("machine_start_gcode",
                                                 new ConfigOptionString("this_is_elegoo_test_machine_start_gcode_mock"));
...
{
    po::options_description desc("Elegoo Profile Validator\nUsage");
    // clang-format off
